...
class moving_avg(nn.Module):
    ...
    def forward(self, x, scale=1):
        if x is None:
            return None
        x = nn.functional.avg_pool1d(x.unsqueeze(1), scale, scale)
        x = x.squeeze(1)
        return x
...
class _NHITSBlock(nn.Module):
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor,
        ...
        # print(insample_y.shape,insample_y[:,:-1].shape)
        try:
            theta = self.layers(insample_y)
        except:
            try:
                zeros = torch.zeros((insample_y.shape[0], 1)).to(insample_y.device)
                insample_y = torch.cat((insample_y, zeros), dim=1)

                theta = self.layers(insample_y) #[:,:self.n_mlp_units[0]]
            except:
                theta = self.layers(insample_y[:,:self.n_mlp_units[0]])  # [:,:self.n_mlp_units[0]]
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        for scale in self.scales:
            tmp_n_mlp_units = [[512//scale, 512//scale], [512//scale, 512//scale], [512//scale, 512//scale]]
            blocks = self.create_stack(stack_types=stack_types,
                                       n_blocks=n_blocks,
                                       n_time_in=(self.seq_len+self.pred_len)//scale,
                                       n_time_out=self.pred_len//scale,
                                       n_x=n_x,
                                       n_x_hidden=n_x_hidden,
                                       n_s=n_s,
                                       n_s_hidden=n_s_hidden,
                                       n_layers=n_layers,
                                       n_mlp_units=tmp_n_mlp_units,
            ...
            self.blocks.append(t.nn.ModuleList(blocks))
        self.blocks = t.nn.ModuleList(self.blocks)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, batch_x_embid=None,bach_x_cov=None,batch_y_embid=None,return_decomposition=False, **kwargs):
        ...
        if D_norm:
            ...
            # seq_last=torch.max(x,dim=1)[0].unsqueeze(1)
            x_enc=x_enc-seq_last

        # insample
        insample_y = x_enc.permute([0, 2, 1]).reshape([-1, self.seq_len])
        insample_x_t = x_mark_enc.permute([0, 2, 1]).reshape([-1, self.seq_len])
        insample_mask = t.ones_like(insample_y)


        # outsample
        outsample_y = x_dec[:, -self.pred_len:, :].permute([0, 2, 1]).reshape([-1, self.pred_len])
        outsample_x_t = x_mark_dec[:, -self.pred_len:, :].permute([0, 2, 1]).reshape([-1, self.pred_len])
        ...
        # time.sleep(500)
        outputs = []
        for scale_indx, scale in enumerate(scales):
            # print('...')
            # time.sleep(500)
            # Manage Scales
            insample_y_scale = self.mv(insample_y, scale)
            if scale == scales[0]: # initialization
                outsample_y_scale = self.mv(outsample_y, scale)
            else: # upsampling of the output from the previous steps
                outsample_y_scale = self.upsample(forecast.unsqueeze(1).detach()).squeeze(1)
            insample_x_t_scale = insample_x_t[:, scale//2::scale]
            insample_mask_scale = insample_mask[:, scale//2::scale]
            outsample_mask_scale = outsample_mask[:, scale//2::scale]
            outsample_x_t_scale = outsample_x_t[:, scale//2::scale]

            # Concat Inputs
            insample_y_scale = t.cat((insample_y_scale, outsample_y_scale), 1)
            insample_x_t_scale = t.cat((insample_x_t_scale, outsample_x_t_scale), 1)
            insample_mask_scale = t.cat((insample_mask_scale, outsample_mask_scale), 1)

            # Cross-scale normalization
            if scale == scales[0]: # initialization
                mean = insample_y_scale[:, :-outsample_y_scale.shape[1]].mean(1).unsqueeze(1)
                insample_y_scale[:, :-outsample_y_scale.shape[1]] = insample_y_scale[:, :-outsample_y_scale.shape[1]] - mean
            else:
                mean = insample_y_scale.mean(1).unsqueeze(1)
                insample_y_scale = insample_y_scale - mean

            if return_decomposition:
                forecast, block_forecasts = self.forecast_decomposition(insample_y=insample_y_scale,
                                                                        insample_x_t=insample_x_t_scale,
                                                                        insample_mask=insample_mask_scale,
                                                                        outsample_x_t=outsample_x_t_scale,
                                                                        x_s=S,
                                                                        scale=scale_indx)
                forecast = forecast + mean
                output = forecast.view([x_dec.shape[0], -1, self.n_time_out//scale]).permute([0, 2, 1])
                outputs.append(output)
            else:
                forecast = self.forecast(insample_y=insample_y_scale,
                                         insample_x_t=insample_x_t_scale,
                                         insample_mask=insample_mask_scale,
                                         outsample_x_t=outsample_x_t_scale,
                                         x_s=S, 
                                         scale=scale_indx)
                forecast = forecast + mean
                output = forecast.view([x_dec.shape[0], -1, self.pred_len//scale]).permute([0, 2, 1])
                if D_norm:
                    outputs.append(output+seq_last)
                else:
                    outputs.append(output)
        return outputs

    def forecast(self, insample_y: t.Tensor, insample_x_t: t.Tensor, insample_mask: t.Tensor,
                 outsample_x_t: t.Tensor, x_s: t.Tensor, scale):
        ...
        for i, block in enumerate(self.blocks[scale]):
            backcast, block_forecast = block(insample_y=residuals, insample_x_t=insample_x_t,
                                             outsample_x_t=outsample_x_t, x_s=x_s)
            # print(residuals.shape,backcast.shape,insample_mask.shape)
            dim_ = [residuals.shape[1], backcast.shape[1], insample_mask.shape[1]]
            if len(set(dim_)) != 1:
                # insample_mask=insample_mask[:,:residuals.shape[1]]
                zeros = torch.zeros((residuals.shape[0], 1)).to(residuals.device)
                if residuals.shape[1]<max(dim_) and backcast.shape[1]<max(dim_):
                    residuals=torch.cat((residuals, zeros), dim=1)
                    backcast = torch.cat((backcast, zeros), dim=1)
                if residuals.shape[1]<max(dim_):
                    residuals = torch.cat((residuals, zeros), dim=1)
                if backcast.shape[1] < max(dim_):
                    backcast = torch.cat((backcast, zeros), dim=1)
                if insample_mask.shape[1]<max(dim_):
                    insample_mask = torch.cat((insample_mask, zeros), dim=1)
                # else:
                #     insample_mask = insample_mask[:, :residuals.shape[1]]
            # print(residuals.shape, backcast.shape, insample_mask.shape,dim_)
            # time.sleep(500)
